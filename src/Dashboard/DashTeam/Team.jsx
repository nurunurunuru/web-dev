...
const Team = () => {
    ...
    const teamPost = (e) => {
        ...
        fetch('https://api.cloudinary.com/v1_1/doke1aoji/image/upload', {
            ...
            .then(data => {
                if (data.url) {
                    const teamMember = {
                        fullName, email, phone, facebook, whatsapp, git, designation, role,
                        photo: data.url
                    }
    ...
    const teamUpdate = (e, id) => {
        ...
        if (avtr != undefined && avtr != "") {
            ...
            const teamMember = {
                fullName, email, phone, facebook, whatsapp, git, designation, role, photo: updateNeed.photo
            }
        ...
        <div className={`dashTeam dash-w-${width}`}>
            ...
            <div className="team-control">
                <div className="team-list-edit">
                            ...
                            <tbody>
                                {
                                    teamMember.map((team, i) => <tr key={i}>
                                        ...
                                        <td>{team?.designation ? team.designation : "Not Added"}</td>
                                        <td>{team?.role ? team.role : "Not Added"}</td>
                                        <td className='flex gap-5 item-center justify-center h-100'>
                                            <button onClick={() => editTeam(team)} className='text-primary'><FaRegEdit /></button>
                    ...
                    <div style={{ transform: `scale(${scaleChange})` }} className={`update-team ${scaleChange == 0 ? "h-0" : "h-auto"}`}>
                        <h4 className='text-dark font-secondary flex justify-between'>Update Team Member <IoClose onClick={() => closeUpdate()} /></h4>
                        <form onSubmit={(e) => teamUpdate(e, updateNeed._id)}>
                            <div className="team-input">
                                <label htmlFor="name">Full Name</label>
                                <input type="text" id='name' defaultValue={updateNeed?.fullName} name='fullName' />
                            </div>
                            <div className="team-input">
                                <label htmlFor="email">Email</label>
                                <input type="email" id='email' defaultValue={updateNeed?.email} name='email' />
                            </div>
                            <div className="team-input">
                                <label htmlFor="phone">Phone</label>
                                <input type="number" id='phone' defaultValue={updateNeed?.phone} name='phone' />
                            </div>
                            <div className="team-input">
                                <label htmlFor="facebook">Facebook Link</label>
                                <input type="text" id='facebook' defaultValue={updateNeed?.facebook} name='facebook' />
                            </div>
                            <div className="team-input">
                                <label htmlFor="whatsapp">WhatsApp Link</label>
                                <input type="text" id='whatsapp' defaultValue={updateNeed?.whatsapp} name='whatsapp' />
                            </div>
                            <div className="team-input">
                                <label htmlFor="git">GitHub Link</label>
                                <input type="text" id='git' defaultValue={updateNeed?.git} name='git' />
                            </div>
                            <div className="team-input">
                                <label htmlFor="designation">Designation</label>
                                <input type="text" id='designation' defaultValue={updateNeed?.designation} name='designation' />
                            </div>
                            <div className="team-input">
                                <label htmlFor="role">Role</label>
                                <input type="text" id='role' defaultValue={updateNeed?.role} name='role' />
                            </div>
                            <div className="team-input">
                                <label htmlFor="photo">
                                    Photo
                                ...
                                <div className="photo-box" onClick={() => document.querySelector('.photo').click()}>
                                    {
                                        disabledBox !== false ? undefined : <input onChange={(e) => {
                                            setAvtr(e.target.files[0]);
                                            setPrev(URL.createObjectURL(e.target.files[0]));
                                        }} type="file" name="photo2" id="image2" draggable='true' className='photo' hidden />
                                    }

                                    {
                                        avtr ? <img className='preview' src={prev} alt='Web Developer' /> : <img src={updateNeed?.photo} />
                                    }
                                </div>
                            </div>
                            ...
                            <div className="photo-box" onClick={() => document.querySelector('.photo').click()}>
                                {
                                    disabledBox ?
                                        ...
                                        }} type="file" name="photo" id="image" draggable='true' className='photo' hidden /> : undefined
                                }
                                {
