...
const DashService = () => {
    ...
    const [width, setWidth] = useOutletContext();
                ...
                if (data.url) {
                    fetch('https://api.cloudinary.com/v1_1/doke1aoji/image/upload', {
                        ...
                        .then(data2 => {
                            if (data2.url) {
                                const service = {
                                    title, fDesc, sDesc, teams, feature, category, photo: data.url, midImg: data2.url
                                }
    ...
    return (
        <div className={`dashTeam dash-w-${width}`}>
            ...
            <div className="service-control">
                <div className="service-action-edit">
                            ...
                            <tbody>
                                {
                                    services.map((service, i) => <tr key={i}>
                                        <th>{i + 1}</th>
                                        <td><img src={service.photo} alt="web developer" /></td>
                                        <td className='name'>{service.title}</td>
                                        <td>{service?.category ? `${service.category.substring(0, 20)}...` : "Not Added"}</td>
                                        <td>{service?.fDesc ? `${service.fDesc.substring(0, 20)}...` : "Not Added"}</td>
                                        <td>{service?.sDesc ? `${service.sDesc.substring(0, 20)}...` : "Not Added"}</td>
                                        <td>{service?.midImg ? <img src={service.midImg} alt="web developer" /> : "Not Added"}</td>
                                        <td>{service?.teams ? service.teams.map((working, i) => <li className='flex' style={{ listStyle: "none" }} key={i}>
                                            <p>{working.name.substring(0, 8)}..</p>
                                            <p>{working.description.substring(0, 8)} ..</p>
                                        </li>) : "Not Added"}</td>
                                        <td>{service?.feature ? <ul className='flex'>
                                            {service.feature.map((feat, i) => <li key={i}>{feat.substring(0, 5)}..</li>)}
                                        </ul> : "Not Added"}</td>
                                        <td className='flex gap-5 item-center justify-center'>
                                            <button onClick={() => editService(service)} className='text-primary'>
                                                <FaRegEdit />
                                            </button>
                    ...
                    <div style={{ transform: `scale(${styleScale})` }} className={`update-service ${styleScale == 0 ? "h-0" : "h-auto"}`}>
                        <h4 className='text-dark font-secondary flex justify-between'>Update Service <IoClose onClick={() => closeService()} /></h4>
                        <form onSubmit={(e) => serviceUpdate(e, updateNeed._id)}>
                            <div className="team-input">
                                <label htmlFor="title">Title</label>
                                <input type="text" id='title' defaultValue={updateNeed?.title} name='title' />
                            </div>
                            <div className="team-input">
                                <label htmlFor="fDesc">1st Description</label>
                                <textarea id='fDesc' defaultValue={updateNeed?.fDesc} name='fDesc' />
                            </div>
                            <div className="team-input">
                                <label htmlFor="sDesc">2nd Description</label>
                                <textarea id='sDesc' defaultValue={updateNeed?.sDesc} name='sDesc' />
                            </div>
                ...
                <div className="add-team">
                    ...
                    <form onSubmit={servicePost}>
                        ...
                        <div className="team-input">
                            <label htmlFor="working" className='label'>Working With</label>
                            {
                                filedInput.map((data, i) => {
                                    return (
                                        <div key={i}>
                                            <label htmlFor="team-name">* Member Name</label>
                                            <input type="text" name='team-name' value={data.name} onChange={(e) => changeInput(e, i, 'name')} />
                                            <br />
                                            <br />
                                            <label htmlFor="team-desc">* Member Description (Short)</label>
                                            <textarea name="team-desc" value={data.description} onChange={(e) => changeInput(e, i, 'description')}></textarea>
                                        </div>
                                    )
                                })
                            }
                            <p className='btn-primary add-btn' onClick={() => addInput()}>Add Team</p>
                        </div>
                        <div className="team-input">
                            <label htmlFor="working" className='label'>Features</label>
                            {
                                filedInputF.map((data, i) => {
                                    return (
                                        <div key={i}>
                                            <input type="text" name='feature' onChange={(e) => changeInputF(e, i)} />
                                        </div>
                                    )
                                })
                            }
                            <p className='btn-primary add-btn' onClick={() => addInputF()}>Add Feature</p>
                        </div>
                        <div className="team-input">
                        ...
                        <div className="team-input">
                            ...
                            <div className="photo-box" onClick={() => document.querySelector('.photo').click()}>
                                ...
                                {
                                    avatar ? <img className='preview' src={preview} alt='Web Developer' /> : <>
                        ...
                        <div className="team-input">
                            ...
                            <div className="photo-box" onClick={() => document.querySelector('.photo2').click()}>
                                <input onChange={(e) => {
                                    setAvatar2(e.target.files[0]);
                                    setPreview2(URL.createObjectURL(e.target.files[0]));
                                }} type="file" name="photo2" id="image2" draggable='true' className='photo2' hidden />
                                {
                                    avatar2 ? <img className='preview' src={preview2} alt='Web Developer' /> : <>
